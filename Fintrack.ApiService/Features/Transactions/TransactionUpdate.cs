...
public sealed class TransactionUpdate
{
       ...
       ) : ICommand<Response>
    {
        ...
        [JsonIgnore]
        public TransactionId Id { get; set; } = default!;
    }
    ...
    public class Validator : AbstractValidator<Request>
    {
        public Validator()
        {
            ...
            RuleFor(x => x.Note).MaximumLength(200).MinimumLength(1);
    ...
    public class TransactionUpdateEndpoint : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            app.MapPut("/transactions/{id}", async (Guid id, ISender sender, HttpContext context, Request request) =>
            {
                request.UserId = context.GetUserId(context.Request.Query["userId"].FirstOrDefault());
                request.Id = TransactionId.From(id);
